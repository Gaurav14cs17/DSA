...
See the matrix layout in the visual overview — $O(1)$ edge lookup, $O(V^2)$ space.

---

### Adjacency List Example

See the list layout in the visual overview — $O(\deg(v))$ neighbor iteration, $O(V+E)$ space.

### Matrix vs List Comparison

![Matrix vs List Comparison](./images/adjacency_comparison.png)
...
See the weighted example in the visual overview — store `(neighbor, weight)` pairs per node.
